...
//forEach second argument is the index, third argument is the array itself forEach is calledon.
orderTotals.forEach((order, i) =>
  console.log(`person with order number: ${i} has spent ${order} `)
);
...
inventoryTotalWorth = inventory.reduce((acc, curr) => {
  return acc + curr.price;
}, 0);

console.log(inventoryTotalWorth);

//Count every letter and number in the text.

const text = `
MDN will be in maintenance mode for a brief period Tuesday June 2, from around 10:00 AM until no later than 12:00 PM Pacific (in UTC, Tuesday June 2, 5:00 PM to 7:00 PM). There should be no disruption in service.
...
//remove everything thats not a letter or a number
function onlyLettersAndNumbers(character) {
  return character.match(/[a-z0-9]/);
}
//count how many times each character appears in the text.
//reduce?
function occuranceCounter(count, character) {
...
const result = text
  .toLocaleLowerCase()
  .split('')
  .filter(onlyLettersAndNumbers)
  .reduce(occuranceCounter, {});
